...
<pre>
• サービスの自動起動の無効化:
systemctl disable scvmmguestagent

• サービスの自動起動の有効化:
systemctl enable scvmmguestagent

・VMM ゲスト エージェントのアンインストール:
./install -r
(VMM ゲスト エージェントのインストールで使用した [install] ファイルを使用します。
VMM ゲスト エージェントの tar ファイルは不要です。)
</pre>

### ３．VMM ゲスト エージェントのアップデート方法
