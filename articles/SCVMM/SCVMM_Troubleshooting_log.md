...
3. PowerShell Script "[Get-SCVMM-Environment-Data.zip](Get-SCVMM-Environment-Data.zip)" をダウンロードし、SCVMM 管理サーバー上の任意のフォルダーへ保存して解凍します。
4. スタートメニューより [Virtual Machine Manager Command Shell] を実行します。
5. Script を解凍したフォルダーへ移動し、以下のコマンドを実行します。
 ```
...
※ 補足
本 Script では、SCVMM サーバー情報、管理対象コンピューター情報、ストレージ情報、ネットワーク情報等を取得し、ZIP ファイルとして出力します。
