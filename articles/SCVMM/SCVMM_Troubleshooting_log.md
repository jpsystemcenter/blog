...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><a href="#No-3-SCVMM-環境情報">SCVMM 環境情報</a></td>
      <td>お客様環境を把握するための情報です。取得対象は、SCVMM サーバー情報, ホスト情報, 仮想マシン情報、ライブラリサーバー情報、ストレージ情報、仮装ネットワーク情報、直近のジョブ履歴情報などです。</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><a href="#No-7-VMM ゲスト エージェントのログ">VMM ゲスト エージェントのログ</a></td>
      <td>Linux VM テンプレートから仮想マシンをデプロイできない、デプロイがタイムアウトする現象の調査で使用します。</td>
    </tr>
  </tbody>
</table>
...
## [No.3 SCVMM 環境情報]
問題が発生している環境やご利用状況を把握するため、SCVMM の構成情報を取得させていただいております。
簡易的に情報を取得できる PowerShell Script をご用意しておりますので、以下の手順で情報を取得し、出力された ZIP ファイルをご提供ください。
 
<b>■ 取得手順</b>
1. SCVMM 管理サーバーに管理者権限を持つユーザーでサインインします。
2. C ドライブ直下に C:\temp フォルダーを作成します。（既に存在する場合は不要です。）
3. PowerShell スクリプト "[Get-SCVMM-Environment-Data.zip](Get-SCVMM-Environment-Data.zip)" をダウンロードし、SCVMM 管理サーバー上の任意のフォルダーへ保存して解凍します。
4. スタートメニューより [Virtual Machine Manager Command Shell] を実行します。
5. スクリプトを解凍したフォルダーへ移動し、以下のコマンドを実行します。
 ```
 .\Get-SCVMM-Environment-Data.ps1
 ```
6. SCVMM 管理サーバーの FQDN (例：scvmm2025.domain1.local) を入力し、Enter キーを押下します。
7. 実行完了後、C:\temp 配下に「SCVMM_Data_YYYYMMDD-HHMMSS.zip」が出力されます。
8. 出力された「SCVMM_Data_YYYYMMDD-HHMMSS.zip」を弊社までお送りください。

※ 実行例
本スクリプトをでは、SCVMM サーバー情報、管理対象コンピューター情報、ストレージ情報、ネットワーク情報等を取得し、ZIP ファイルとして出力します。
![](009.png)
...
Linux VM テンプレートから仮想マシンのデプロイに失敗する場合、VMM ゲスト エージェントのログ確認が必要となります。
また、syslog も併せて連携をいただくことで、VMM ゲスト エージェントのサービスやプロセスに問題が発生していた場合、その特定が可能となる場合がございます。
以下のディレクトリを丸ごと、もしくはファイルを、デプロイが失敗した Linux 仮想マシンから採取します。

採取するディレクトリ:
 ```
/var/opt/microsoft/scvmmguestagent/log
 ```
...
デプロイが失敗した場合でも、その仮想マシンの OS にログインすることは可能です。
その性質上デプロイ処理中に採取するのではなく、失敗が確認された段階で採取します。